...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^discovery_") ; then
        DISCOVERY_BUILD=$(echo -n $1 | sed -e 's/^discovery_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $DISCOVERY_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        DISCOVERY_BUILD=
    fi
    export DISCOVERY_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${DISCOVERY_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the discovery model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch discovery_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the discovery GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/discovery/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/discovery/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $DISCOVERY_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.discovery.device | grep -q "$DISCOVERY_BUILD");
    then
        ...
        echo "The connected device does not appear to be $DISCOVERY_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.discovery.device | grep -q "$DISCOVERY_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/discovery/build/envsetup.sh
